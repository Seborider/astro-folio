...
export default defineType({
  ...
  fields: [
    ...
    defineField({ name: "archiveHeading", title: "Archive carousel heading", type: "localeString", description: "Heading above the home-page footer carousel." }),
    defineField({ name: "location", title: "Location", type: "localeString", description: 'Footer "Standort" column, e.g. "Kopenhagen".' }),
    defineField({ name: "timezoneLabel", title: "Timezone label", type: "localeString", description: 'e.g. "UTC+1". Display only — the clock itself is fixed to CPH.' }),
    defineField({ name: "copyright", title: "Copyright line", type: "localeString" }),
    defineField({ name: "colophon", title: "Colophon line", type: "localeString", description: 'e.g. "Built with Astro · GSAP · Lenis · WebGL".' }),
    defineField({ name: "footerNote", title: "Footer note", type: "localeString", description: "Third line in the footer bottom row." }),
  ],

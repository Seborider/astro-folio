...
function readTokens() {
  ...
  return Object.fromEntries(
    TOKEN_NAMES.map((n) => [n, cs.getPropertyValue("--" + n).trim()]),
  ) as typeof TWEAK_DEFAULTS;
}
...
function FolioTweaks({ locale }: { locale: Locale }) {
  ...
  const change = (name: keyof typeof TWEAK_DEFAULTS) => (v: string) => {
    setTweak(name, v);

...
//
// A floating glass panel with its own always-visible toggle button. State lives
// in React (useTweaks) and is applied live; there is no host/persistence layer.
//
// Usage (rendered as an Astro island):
//
//   const TWEAK_DEFAULTS = { "primaryColor": "#D97757" };
//
//   function App() {
...
//   }
//
// For color tweaks always curate 3-4 options rather than a free picker; an option
// can also be a whole 2–5 color palette (the stored value is the array).
/* END USAGE */
// ─────────────────────────────────────────────────────────────────────────────
...
const __TWEAKS_STYLE = `
  .twk-toggle-btn{position:fixed;right:16px;bottom:16px;z-index:2147483646;
    ...
    letter-spacing:.01em;cursor:default;transition:transform .12s,box-shadow .12s}
  .twk-toggle-btn:hover{transform:scale(var(--dc-inv-zoom,1)) translateY(-1px);
    box-shadow:0 1px 0 rgba(255,255,255,.5) inset,0 16px 48px rgba(0,0,0,.22)}
  .twk-toggle-btn i{width:9px;height:9px;border-radius:50%;
    background:var(--accent,#D97757);box-shadow:0 0 0 .5px rgba(0,0,0,.15)}
...
// Curated color/palette picker. Each option is either a single hex string or an
// array of 1-5 hex strings; the card adapts — a lone color renders solid, a
// palette renders colors[0] as the hero (left ~2/3) with the rest stacked in a
...
export { useTweaks, TweaksPanel, TweakSection, TweakColor };
